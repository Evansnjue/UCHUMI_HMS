...
describe('VisitService (unit) - scaffold', () => {
  it('creates visit and publishes VisitCreated', async () => {
    ...
    const mockPatientRepo: any = { findOne: jest.fn().mockResolvedValue({ id: 'p1' }) };
    const mockDeptRepo: any = { findOne: jest.fn().mockResolvedValue({ id: 'd1', code: 'GEN' }) };
    const mockVisitRepo: any = { create: jest.fn().mockReturnValue({}), save: jest.fn().mockResolvedValue({ id: 'v1' }), findOne: jest.fn().mockResolvedValue(null) };
    const mockStatusRepo: any = { findOne: jest.fn().mockResolvedValue({ id: 's1', name: 'QUEUED' }) };
    const mockQueueRepo: any = { save: jest.fn() };
    ...
    const res = await svc.create({ patientId: 'p1', departmentCode: 'GEN' } as any);
