...
describe('Clinical Admin helpers', () => {
  it('creates and updates drug categories', async () => {
    const catRepo: any = { create: jest.fn().mockReturnValue({}), save: jest.fn().mockResolvedValue({ id: 'c1', name: 'Cat1', dailyLimit: 2 }), find: jest.fn().mockResolvedValue([{ id: 'c1', name: 'Cat1', dailyLimit: 2 }]), update: jest.fn(), findOne: jest.fn().mockResolvedValue({ id: 'c1', name: 'Cat1', dailyLimit: 2 }) };
