...
describe('ClinicalService - prescription limits', () => {
  it('blocks prescription exceeding daily category limit', async () => {
    const consultRepo: any = { findOne: jest.fn().mockResolvedValue({ id: 'c1' }) };
    const drugCategory = { id: 'cat1', name: 'Opioids', dailyLimit: 5 };
    const drug = { id: 'd1', name: 'DrugA', category: drugCategory };
    const drugRepo: any = { findByIds: jest.fn().mockResolvedValue([drug]) };
    // existing prescription with quantity 3
    const presRepo: any = { createQueryBuilder: jest.fn().mockReturnValue({ leftJoinAndSelect: jest.fn().mockReturnThis(), where: jest.fn().mockReturnThis(), andWhere: jest.fn().mockReturnThis(), getMany: jest.fn().mockResolvedValue([{ items: [{ quantity: 3 }] }]) }), save: jest.fn() };
    const itemsRepo: any = { create: jest.fn() };
    const catRepo: any = { findOne: jest.fn().mockResolvedValue(drugCategory) };
    ...
    await expect(svc.createPrescription({ consultationId: 'c1', items: [{ drugId: 'd1', quantity: 3 }] } as any, { id: 'doc1' } as any)).rejects.toBeTruthy();
  });
});
