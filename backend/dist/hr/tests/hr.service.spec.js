...
describe('HRService - unit', () => {
    let svc;
    const empRepo = { create: jest.fn(), save: jest.fn(), findOne: jest.fn(), count: jest.fn(), createQueryBuilder: jest.fn(), find: jest.fn() };
    const attRepo = { create: jest.fn(), save: jest.fn(), findOne: jest.fn(), createQueryBuilder: jest.fn() };
    const payrollRepo = { create: jest.fn(), save: jest.fn() };
    ...
    beforeEach(async () => {
        ...
        svc = new HRService(empRepo, attRepo, payrollRepo, eventBus);
    });
    it('allocates employee numbers and creates employee', async () => {
        ...
        const res = await svc.createEmployee({ firstName: 'E', lastName: 'X', email: 'a@b.com', role: 'Nurse', hireDate: '2026-01-01', salary: 1000 });
    ...
    it('marks late on late checkin', async () => {
        ...
        attRepo.create.mockImplementation((x) => x);
        attRepo.save.mockImplementation(async (x) => ({ ...x, id: 'a1' }));
        // Provide a timestamp after 09:20 UTC to exceed 15 min grace
        const ts = new Date();
        ts.setUTCHours(9, 30, 0, 0);
        const res = await svc.checkIn({ employeeId: 'e2', timestamp: ts.toISOString() });
    ...
    it('calculates overtime on checkout', async () => {
        ...
        const att = { id: 'att1', checkIn, shiftDate: checkIn.toISOString().substring(0, 10), employee: emp };
        attRepo.findOne.mockResolvedValue(att);
        attRepo.save.mockImplementation(async (x) => x);
        // checkout at 18:00 (10 hours -> 2h overtime)
        const out = new Date();
        out.setUTCHours(18, 0, 0, 0);
        const res = await svc.checkOut({ employeeId: 'e3', timestamp: out.toISOString() });
    ...
    it('generates payroll records', async () => {
        ...
        payrollRepo.create.mockImplementation((x) => x);
        payrollRepo.save.mockImplementation(async (x) => ({ ...x, id: 'p1' }));
        const res = await svc.generatePayroll({ periodStart: '2026-01-01', periodEnd: '2026-01-31' });
