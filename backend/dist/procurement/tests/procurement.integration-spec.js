...
describe('Procurement e2e', () => {
    ...
    beforeAll(async () => {
        ...
        await authService.register({ email: testUser.email, firstName: testUser.firstName, lastName: testUser.lastName, password: testUser.password, roles: [role] });
        const res = await request(app.getHttpServer()).post('/auth/login').send({ email: testUser.email, password: testUser.password });
        token = res.body?.accessToken;
    });
    afterAll(async () => { if (app)
        await app.close(); });
