var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var InventoryEventsSubscriber_1;
import { Injectable, Logger } from '@nestjs/common';
import { EventBusService } from '../auth/event-bus.service';
let InventoryEventsSubscriber = InventoryEventsSubscriber_1 = class InventoryEventsSubscriber {
    constructor(eventBus) {
        this.eventBus = eventBus;
        this.logger = new Logger(InventoryEventsSubscriber_1.name);
        this.eventBus.subscribe('StockUpdated', this.handleStockUpdated.bind(this));
        this.eventBus.subscribe('StockLowAlert', this.handleStockLow.bind(this));
    }
    handleStockUpdated(payload) {
        this.logger.log(`Stock updated for item ${payload.itemId} newQty=${payload.newQuantity}`);
    }
    handleStockLow(payload) {
        this.logger.warn(`Low stock alert for item ${payload.itemId} qty=${payload.quantity} threshold=${payload.threshold}`);
        // TODO: integrate with notification service (email/SMS/webhook)
    }
};
InventoryEventsSubscriber = InventoryEventsSubscriber_1 = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [EventBusService])
], InventoryEventsSubscriber);
export { InventoryEventsSubscriber };
//# sourceMappingURL=inventory.events.subscriber.js.map