...
describe('Inventory e2e (authenticated flows)', () => {
    ...
    beforeAll(async () => {
        ...
        deptRepo = moduleFixture.get(getRepositoryToken(Department));
        itemRepo = moduleFixture.get(getRepositoryToken(InventoryItem));
        ...
        await authService.register({ email: testUser.email, firstName: testUser.firstName, lastName: testUser.lastName, password: testUser.password, roles: [adminRole] });
    ...
    it('adds stock successfully', async () => {
        ...
        const dept = deptRepo.create({ name: 'TestDept' + Date.now(), description: 'Test' });
        await deptRepo.save(dept);
        const item = itemRepo.create({ name: 'TestItem', sku: 'TI-1', quantity: 0, department: dept });
    ...
    it('transfers stock between departments', async () => {
        ...
        const destDept = deptRepo.create({ name: 'DestDept' + Date.now(), description: 'Dest' });
        ...
        const dest = await itemRepo.findOne({ where: { name: source.name, batch: source.batch, department: { id: destDept.id } } });
    ...
    it('enforces RBAC for non-admin users', async () => {
        ...
        await authService.register({ email: 'pharm@test.local', firstName: 'Pharm', lastName: 'T', password: 'Pharm1', roles: [pharmRole] });
...
//# sourceMappingURL=inventory.integration-spec.js.map